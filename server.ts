...
// Fallback items in case external API is unreachable or rate limited
...
// API endpoint to fetch festival list
app.get('/api/festivals', async (req, res) => {
  const now = Date.now();

  if (cachedFestivals && (now - lastFetchTime) < CACHE_TTL_MS) {
    return res.json({
      success: true,
      source: 'cache',
      items: cachedFestivals
    });
  }
  ...
  try {
    ...
    // Return cached if available, else return fallback items
    if (cachedFestivals) {
      return res.json({
        success: true,
        source: 'stale_cache',
